...
/*
 * Loongson LS2H platform-bridge register definitions.
 * Vendored from the Loongson kernel tree (loongson-pch.h), GPL-2.0.
 */
...
static struct platform_device *ls2h_fb_pdev;
...
static int __init ls2h_fb_init(void)
{
	...
	while ((pdev = pci_get_class(PCI_CLASS_DISPLAY_VGA << 8, pdev))) {
		...
		if (pdev->vendor == 0x1a03) /* ASpeed */
			return 0;
		if (pdev->vendor == PCI_VENDOR_ID_LOONGSON) /* LS7A DC/GPU */
			return 0;
	}
	...
	/* The LS2H display controller is not reachable through PCI:
	 * register a platform device for it. */
	ls2h_fb_pdev = platform_device_register_simple("ls2h-fb", -1, NULL, 0);
	if (IS_ERR(ls2h_fb_pdev))
		pr_warn("ls2h-fb: failed to register platform device\n");
...
{
	if (!IS_ERR_OR_NULL(ls2h_fb_pdev))
		platform_device_unregister(ls2h_fb_pdev);
